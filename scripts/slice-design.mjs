...
const result = await page.evaluate(
  async ({ dataUrl, minGap, minSection }) => {
    const img = new Image();
    img.src = dataUrl;
    await img.decode();
    const W = img.width;
  ...
  { dataUrl, minGap, minSection },
);

writeFileSync(path.join(outDir, "full.png"), Buffer.from(result.full.split(",")[1], "base64"));
const meta = [];
for (const s of result.sections) {
  const name = `section-${String(s.index).padStart(2, "0")}.png`;
...
console.log(`${meta.length} セクションに切り出しました → ${path.relative(process.cwd(), outDir)}/`);
