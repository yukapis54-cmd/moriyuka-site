...
export function reasonsFor(a: Answers, paletteName: string): DesignReason[] {
  ...
  const reasons: DesignReason[] = [
    ...
    },
    {
      ...
      body: "背景・文字・差し色の3色でほぼ全部を組んでいます。目安は 70:25:5。差し色を5%に抑えるから、そこが「一番強い場所」になります。色数を増やすと、どの色も強くなくなります。グラフや表を載せるときも同じで、全体をグレーの濃淡（見分けられるのは3段階まで）に沈めて、言いたい部分にだけ色を残すと、一番伝わります。",
    },
  ];

  if (a.goal === "shop" || a.goal === "lead") {
    reasons.push({
      title: "一番事務的な画面こそ、丁寧に作る",
      body:
        a.goal === "shop"
          ? "カートや購入フォームは、普通なら一番そっけなくなる場所です。でもお客さんが最後に見るのはそこ。買う直前まで楽しませられると、他の店との差がはっきり出ます。"
          : "問い合わせフォームは、その人が一番迷っている場所です。項目を減らし、入力例を添え、送信後に何が起きるかを書く。ここを丁寧にするだけで、届く件数が変わります。",
    });
  }
